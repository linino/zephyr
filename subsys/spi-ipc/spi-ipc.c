...
NET_BUF_POOL_VAR_DEFINE(spi_ipc_pool, 8, 2*1024, NULL);
			      ...
			      struct net_buf **reply)
{
	...
	net_buf_linearize(&header, sizeof(header), request, 0, sizeof(header));
	msg->error = -ETIMEDOUT;
	...
	/* Enqueue buffer */
	net_buf_put(&data->fifo, request);

	if (!spi_ipc_is_request(&header) || !reply)
		/* Discard any reply */
		goto end;
	...
	/* Read reply header */
	*reply = msg->reply ? msg->reply->netbuf : NULL;
end:
	return ret;
}
